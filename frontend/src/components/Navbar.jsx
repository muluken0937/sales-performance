...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-52 w-[calc(100%-13rem)] bg-white border-b border-gray-200 dark:bg-gray-200 dark:border-gray-700 z-50">
      <div className="max-w-full flex items-center justify-between p-2">
        <div className="flex items-center space-x-4 cursor-pointer" onClick={() => navigate("/profile")}>
          {userDetails.image ? (
            ...
            <div className="h-10 w-10 rounded-full bg-gray-400 flex items-center justify-center">
              <span className="text-white text-lg">{userDetails.name?.[0] || "?"}</span>
            </div>
          )}
          <div className="flex flex-col">
            <span className="text-sm font-bold text-gray-800 dark:text-blue-900">{userDetails.name || "Unknown User"}</span>
            <span className="text-xs text-gray-600 dark:text-gray-800">{userDetails.role}</span>
          </div>
        </div>
