...
//     // Disable if isPendingApproval is true
...
//                       }
//                     }}
//                     className={`form-checkbox h-5 w-5 ${customer.isPendingApproval ? "text-blue-600" : "text-blue-600"}`} // Always blue if not pending
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        ...
        // If the user is a SalesUser, fetch only their customers
        if (role === "SalesUser") {
          url += `?userId=${loggedInUserId}`; // Assumes your API supports this query parameter
        }
            ...
            <tr key={customer._id} className="hover:bg-gray-700 cursor-pointer">
              ...
              <td className="py-2 px-4">
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    checked={role === "SalesUser" ? customer.paidStatus : customer.paidStatus}
                    onChange={() => {
                      if (role === "SalesUser" || !customer.isPendingApproval) {
                        handleCheckboxChange(customer._id, "paidStatus");
                      }
              ...
              <td className="py-2 px-4">
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    checked={role === "SalesUser" ? customer.visitStatus : customer.visitStatus}
                    onChange={() => {
                      if (role === "SalesUser" || !customer.isPendingApproval) {
                        handleCheckboxChange(customer._id, "visitStatus");
                      }
